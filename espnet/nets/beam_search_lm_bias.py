...
class Hypothesis(NamedTuple):
    ...
    lid_decision: Dict[str, Any] = dict()
...
class BeamSearch(torch.nn.Module):
    ...
    def score_full(
        ...
        for k, d in self.full_scorers.items():
            if k == "lm":
                ...
                scores[k], states[k] = d.score(hyp.yseq, hyp.states[k], x)
        return scores, states, lid_decisions
    ...
    def merge_lid(self, previous_lid: Any, new_lid: Any, length: Any) -> Any:
        merged_lid = dict()
        # merged_lid[length] = new_lid
        for k, v in previous_lid.items():
            merged_lid[k] = v
        merged_lid[length] = new_lid
        return merged_lid
    ...
    def search(
        ...
        for hyp in running_hyps:
            ...
            for j, part_j in zip(*self.beam(weighted_scores, part_ids)):
                ...
                # logging.info("best_hyps: {} {}".format(hyp.yseq, len(lid_decisions)))
                best_hyps.append(
                    Hypothesis(
                        ...
                        lid_decision=self.merge_lid(
                            hyp.lid_decision, lid_decisions, len(hyp.yseq)
                        ),
                    )
                )
    ...
    def forward(
        ...
        lid_list = list()
        for k, v in best.lid_decision.items():
            logging.info("token {} -- Language log probability {}".format(str(k), v))
            if torch.argmax(v) == 0:
                lid_list.append("E")
            elif torch.argmax(v) == 1:
                lid_list.append("M")
            elif torch.argmax(v) == 3:
                lid_list.append("F")
        logging.info("lid list: {}".format(lid_list))
        logging.info("token list: {}".format(best.yseq))
