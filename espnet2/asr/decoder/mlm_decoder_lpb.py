...
class MLMLPBDecoder(AbsDecoder):
    def __init__(
        self,
        vocab_size: int,
        lid_vocab_size: int,
        ...
        self.embed2 = torch.nn.Sequential(
            torch.nn.Embedding(vocab_size, attention_dim),
            pos_enc_class(attention_dim, positional_dropout_rate),
        )
        ...
        self.after_norm_concat = LayerNorm(attention_dim)
        ...
        self.proj_concat = torch.nn.Linear(attention_dim+lid_vocab_size, attention_dim)
    ...
    def forward(
        ...
        x_lid = self.embed2(tgt)

        x_lid, tgt_mask_lid, memory_lid, memory_mask_lid = self.lid_decoders(
            x_lid, tgt_mask, memory, memory_mask
        )
        
        x_lid = self.after_norm_lid(x_lid)
        x_lid = self.output_lid_layer(x_lid)
        x_lid = torch.nn.functional.softmax(x_lid, dim=-1)

        x = self.embed(tgt)
        x = torch.cat((x, x_lid), dim=-1)
        x = self.proj_concat(x)
        x = self.after_norm_concat(x)
        x, tgt_mask_asr, memory_asr, memory_mask_asr = self.decoders(
