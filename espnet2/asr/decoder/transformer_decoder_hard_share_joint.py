...
class BaseTransformerDecoder(AbsDecoder, BatchScorerInterface):
    ...
    def forward(
        ...
        lid_post = torch.where(lid_post<0.7, lid_post, torch.scalar_tensor(1).float().type_as(x)).type_as(x)
        # lid_post = torch.where(lid_post>0.1, lid_post, torch.scalar_tensor(0).float().type_as(x)).type_as(x)
