...
from egs2.seame.asr1.local.preprocess_lid import transform_text_to_lang
...
class CommonPreprocessor(AbsPreprocessor):
    def __init__(
        ...
        token_lid_list: Union[Path, str, Iterable[str]] = None,
    ...
    def _text_process(
        ...
        if self.text_name in data and self.tokenizer is not None:
            ...
            tokens_lid = self.token_to_lid(tokens)
            text_ints = self.token_id_converter.tokens2ids(tokens)
            text_lid_ints = self.token_id_converter_lid.tokens2ids(tokens_lid)
            data[self.text_name] = np.array(text_ints, dtype=np.int64)
            data[self.lid_name] = np.array(text_lid_ints, dtype=np.int64)
        assert check_return_type(data)
        return data

    def token_to_lid(self, tokens):
        return transform_text_to_lang(tokens)
